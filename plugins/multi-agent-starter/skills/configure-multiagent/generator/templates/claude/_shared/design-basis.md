...
- **D4** gemini 백엔드 = **Antigravity `agy` CLI**(`_shared/backends.json` 정본, 디스패처 `_shared/adapters/call_worker.sh`). 기본 모델 = `gemini-3.1-pro-high`, 폴백 없음. 최신 agy CLI는 `--model` per-call 핀을 지원하므로 디스패처 argv가 `backends.json` 선언과 일치해야 한다. 옛 프록시 브리지는 폐기한다.
...
- **D7** 모델 식별자 표기 정책 = Codex와 Claude는 환경 별칭을 따르고, Gemini worker는 `backends.json` 선언과 agy `--model` argv를 함께 `gemini-3.1-pro-high`로 핀한다. 세부 정책의 정본은 D4다.
