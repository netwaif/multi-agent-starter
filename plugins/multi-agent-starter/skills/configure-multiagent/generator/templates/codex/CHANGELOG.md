...
### Changed
- **backends.json `schema_version` 1 → 2**: `workers` 맵 → `providers` + `roles`(provider 바인딩 + `staffing` + `desk`). 디스패처는 `roles→providers` 해석. 역할별 프로바이더 교체 = `roles` 한 줄. 폐기 `workers` 맵 금지. (design-basis D7 / system-invariants INV10)
