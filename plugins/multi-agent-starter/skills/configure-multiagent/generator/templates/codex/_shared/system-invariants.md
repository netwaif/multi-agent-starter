...
| INV9 | gemini 백엔드가 `agy` CLI이고 `--model gemini-3.1-pro-high`를 실제 argv에 고정; 옛 프록시 활성호출 없음 |
| INV13 | claude-critic이 `claude --print`, `cwd_policy: isolated_tmp`, `--add-dir @target_repo`, `--tools Read,Glob,Grep`, slash-command 비활성 계약을 사용함 |
