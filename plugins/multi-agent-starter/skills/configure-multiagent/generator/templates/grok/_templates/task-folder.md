...
모든 worker 호출은 `_shared/backends.json` 정본 + 디스패처 `bash _shared/adapters/call_worker.sh <role> <brief-file>` → **envelope v2**(JSON). Orchestrator는 envelope의 `output.stdout_path`가 가리키는 파일 내용을 `result.md`에 기록한다(원문이 짧으면 `output.stdout_preview_sanitized`도 가능). **성공 여부는 최상위 `ok`(true/false) 또는 파생 `legacy_status`(`ok`/`error`/`empty`/`timeout`)로 판정** — 옛 `.stdout` 키는 없어졌다(v4 breaking). 호출 전 승인.
...
- **codex-critic**: 백엔드 `codex` CLI. 독립 비평(교차벤더).
- **gemini**: 백엔드 Antigravity `agy` CLI(폴백 api). [multimodal] 이미지·스크린샷·장문서 분석.
- 최신성이 걸린 web/X 사실 확인은 워커가 아니라 오케스트레이터(grok)가 네이티브 web_search/x_search로 직접 수행한다(grok-intel/grok-critic 워커 없음).
...
큰 산출물은 `tasks/<task>/artifacts/`에 저장하고, `result.md`에는 경로만 기록한다. 오케스트레이터가 직접 수행한 web/X 검색 근거도 여기에 스냅샷한다.
