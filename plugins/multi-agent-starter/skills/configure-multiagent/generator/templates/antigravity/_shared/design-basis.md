...
- **D9 payload 분리(brief 한도 밖 동봉 자료)** = 디스패처 `call_worker.sh <role> <brief> [payload]` 3번째 인자 — 대용량 자료는 brief에 인라인하지 않고 `sources/` packet 파일로 동봉(디스패처가 결합, `--merged-preview`로 검증). brief 불변식(inline 금지·1200자) 예외 없음. 아울러 backends.json에서 디스패처가 읽지 않는 선언(`write_policy`·`non_interactive`)을 제거 — 선언된 능력은 구현이 있을 때만 등록(거짓 안전신호 금지). (2026-07-24)
- **D10 집행층 코드화(기계 판정 규칙 → 셸 스크립트)** = 승인 게이트·brief 위치/한도·외부 쓰기 4조건·status↔log 정합처럼 기계로 판정 가능한 규칙은 코드가 집행하고, 산문은 스펙·판단 규칙으로만 남긴다. 정본 = `_shared/adapters/gate.sh`(사전, fail-closed, exit 9)·`scope_check.sh`(사후, 보고만, exit 10/12)·`_shared/reentry-check.sh`(재진입 정합, exit 11) — bash+jq+git, 3 flavor 공유(호스트 훅 미도입 = 벤더 독립). 커버리지: 디스패처 경유는 자동, native/mcp는 호출 전 gate.sh 실행 의무(지침 Task Lifecycle 6). 비파괴: scope 밖 변경은 보고·거부만(자동 revert 없음), 위반은 폴백 없는 최종 실패. scope별 cwd: `none`/`tasks-only`는 cwd=루트, `tasks-only`=현재 작업 폴더만, 패턴 scope만 brief의 target_repo. 승인 정확 일치: 승인 항목의 target_repo·write_scope == brief 값. 스냅샷 계약 = porcelain -z + 내용 해시(ignored·비git은 보장 제외, `scope_check: skipped`). G4 = 한글 비율 30% 초과면 1200자, 아니면 240단어. 재진입 검사는 정정하지 않음([COMPLETE] 뒤 후속 항목 = 정상 재개). 회귀보호 = INV14. (2026-09-06, 루트 D14)
